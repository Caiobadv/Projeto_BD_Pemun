...
public class UsuarioRepository {
    ...
    public int save(Usuario usuario) {
        String sql = "INSERT INTO Usuario (cpf_user, tipo_user, nome_user, instituicao_ensino_user, curso_user, login_user, senha_login_user, email_user, telefone_user, rua_endereco_user, numero_endereco_user, bairro_endereco_user, cep_endereco_user, complemento_endereco_user) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        return jdbcTemplate.update(sql, usuario.getCpf_user(), usuario.getTipo_user(), usuario.getNome_user(), usuario.getInstituicao_ensino_user(), usuario.getCurso_user(), usuario.getLogin_user(), usuario.getSenha_login_user(), usuario.getEmail_user(), usuario.getTelefone_user(), usuario.getRua_endereco_user(), usuario.getNumero_endereco_user(), usuario.getBairro_endereco_user(), usuario.getCep_endereco_user(), usuario.getComplemento_endereco_user());
    }

    public int update(Usuario usuario) {
        String sql = "UPDATE Usuario SET tipo_user = ?, nome_user = ?, instituicao_ensino_user = ?, curso_user = ?, login_user = ?, senha_login_user = ?, email_user = ?, telefone_user = ?, rua_endereco_user = ?, numero_endereco_user = ?, bairro_endereco_user = ?, cep_endereco_user = ?, complemento_endereco_user = ? WHERE cpf_user = ?";
        return jdbcTemplate.update(sql, usuario.getTipo_user(), usuario.getNome_user(), usuario.getInstituicao_ensino_user(), usuario.getCurso_user(), usuario.getLogin_user(), usuario.getSenha_login_user(), usuario.getEmail_user(), usuario.getTelefone_user(), usuario.getRua_endereco_user(), usuario.getNumero_endereco_user(), usuario.getBairro_endereco_user(), usuario.getCep_endereco_user(), usuario.getComplemento_endereco_user(), usuario.getCpf_user());
    }
    ...
    private static class UsuarioRowMapper implements RowMapper<Usuario> {
        ...
        public Usuario mapRow(ResultSet rs, int rowNum) throws SQLException {
            ...
            usuario.setTelefone_user(rs.getString("telefone_user"));
